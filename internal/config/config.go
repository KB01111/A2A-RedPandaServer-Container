...
func Load() (Config, error) {
	...
	oidcConfig, err := loadOIDCConfig(environment)
	if err != nil {
		return Config{}, err
	}
...
func loadOIDCConfig(environment string) (OIDCConfig, error) {
	...
	if issuer == "" && audience == "" {
		for _, key := range []string{
			...
			if strings.TrimSpace(os.Getenv(key)) != "" {
				return OIDCConfig{}, fmt.Errorf("%s requires OIDC_ISSUER and OIDC_AUDIENCE", key)
			}
		}
	...
	for _, algorithm := range algorithms {
		if !AllowedOIDCAlgorithm(algorithm) {
...
// AllowedOIDCAlgorithm reports whether an asymmetric signing algorithm is
// accepted by both configuration loading and runtime verifier construction.
...
func loadDatabaseConfig() (DatabaseConfig, error) {
	...
	for key := range parsedURL.Query() {
		...
		case "password", "sslpassword", "passfile", "service", "servicefile":
			return DatabaseConfig{}, fmt.Errorf("DATABASE_URL must not contain implicit credential sources; use DATABASE_PASSWORD_FILE")
		}
	}
